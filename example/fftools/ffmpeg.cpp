...
#include "libavformat_os_support.h"
}
...
static void close_all_output_streams(OutputStream *ost, OSTFinished this_stream, OSTFinished others)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        (int&)ost2->finished |= ost == ost2 ? this_stream : others;
    }
}
...
static void close_output_stream(OutputStream *ost)
{
    ...
    (int&)ost->finished |= ENCODER_FINISHED;
    if (of->shortest) {
...
static void finish_output_stream(OutputStream *ost)
{
    ...
    (int&)ost->finished = ENCODER_FINISHED | MUXER_FINISHED;

    if (of->shortest) {
        for (i = 0; i < of->ctx->nb_streams; i++)
            (int&)output_streams[of->ost_index + i]->finished = ENCODER_FINISHED | MUXER_FINISHED;
    }
}
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
               ...
               av_ts2str(ist->next_dts), av_ts2timestr(ist->next_dts, (AVRational*)&kAvTimeBase),
               av_ts2str(ist->next_pts), av_ts2timestr(ist->next_pts, (AVRational*)&kAvTimeBase),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
               av_ts2timestr(input_files[ist->file_index]->ts_offset, (AVRational*)&kAvTimeBase));
    }
