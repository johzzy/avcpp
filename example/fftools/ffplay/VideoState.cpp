...
int realloc_texture(SDL_Texture **texture, Uint32 new_format, int new_width, int new_height, SDL_BlendMode blendmode, int init_texture)
{
    ...
    if (!*texture || SDL_QueryTexture(*texture, &format, &access, &w, &h) < 0 || new_width != w || new_height != h || new_format != format) {
        ...
        if (!(*texture = SDL_CreateTexture(VideoState::renderer, new_format, SDL_TEXTUREACCESS_STREAMING, new_width, new_height)))
            return -1;
...
void VideoState::video_audio_display()
{
    ...
    if (!paused) {
        int data_used= show_mode == VideoState::SHOW_MODE_WAVES ? this->width : (2*nb_freq);
        n = 2 * channels;
    ...
    if (this->show_mode == VideoState::SHOW_MODE_WAVES) {
        ...
        if (!this->rdft || !this->rdft_data){
            ...
            FFTSample *data[2];
            SDL_Rect rect = {.x = this->xpos, .y = 0, .w = 1, .h = this->height};
            uint32_t *pixels;
            int pitch;
            for (ch = 0; ch < nb_display_channels; ch++) {
...
int VideoState::stream_component_open(int stream_index)
{
    ...
    switch (avctx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            if ((ret = this->auddec.decoder_start(flush_pkt, audio_thread, "audio_decoder", this)) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            this->viddec.decoder_init(avctx, &this->videoq, this->continue_read_thread);
            if ((ret = this->viddec.decoder_start(flush_pkt, video_thread, "video_decoder", this)) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            this->subdec.decoder_init(avctx, &this->subtitleq, this->continue_read_thread);
            if ((ret = this->subdec.decoder_start(flush_pkt, subtitle_thread, "subtitle_decoder", this)) < 0)
                goto out;
            break;
        default:
            break;
...
void VideoState::toggle_audio_display()
{
    ...
    } while (next != this->show_mode && (next == VideoState::SHOW_MODE_VIDEO && !this->video_st || next != VideoState::SHOW_MODE_VIDEO && !this->audio_st));
...
void VideoState::refresh_loop_wait_event(SDL_Event *event)
{
    ...
    while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
        ...
        if (this->show_mode != VideoState::SHOW_MODE_NONE && (!this->paused || this->force_refresh))
...
void VideoState::seek_chapter(int incr)
{
    ...
    stream_seek(av_rescale_q(this->ic->chapters[i]->start, this->ic->chapters[i]->time_base,
                                 AV_TIME_BASE_Q), 0, 0);
}
            ...
            case SDL_KEYDOWN:
                ...
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_t:
                        ...
                        break;
                    case SDLK_w:
                        if (show_mode == VideoState::SHOW_MODE_VIDEO && vfilter_idx < nb_vfilters - 1) {
    ...
    if (is->video_st) {
        ...
        if (is->pictq.NbRemaining() == 0) {
            ...
            if (is->pictq.NbRemaining() > 1) {
                ...
                if(!is->step && (framedrop>0 || (framedrop && is->get_master_sync_type() != AV_SYNC_VIDEO_MASTER)) && time > is->frame_timer + duration){
                    is->frame_drops_late++;
                    is->pictq.Next();
                    goto retry;
                }
        ...
        int VideoState::video_thread(void *arg)
        {
            ...
            for (;;) {
                ...
                if (   last_w != frame->width
                    ...
                    if ((ret = is->configure_video_filters(graph, is->vfilters_list ? is->vfilters_list[is->vfilter_idx] : NULL, frame)) < 0) {
                        SDL_Event event;
